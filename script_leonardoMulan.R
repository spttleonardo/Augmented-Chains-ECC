...
ECCMulan <- function(train, test, mulan, chains=NA){
  
  atualdir <- getwd()
  setwd(tempdir())
  
  # escreve os arquivos de train e test
  write_arff(train, "train_file_mulan", write.xml = T)
  write_arff(test, "test_file_mulan", write.xml = T)

  # chama o mulan
  if (is.na(chains)){
    mulanstr <- paste(mulan, " -t ", "train_file_mulan.arff -T ", "test_file_mulan.arff -x ",
                  "train_file_mulan.xml -o out.csv -c J48 -a ECC -u -b 75", sep = "")
  }else{
    mulanstr <- paste(mulan, " -t ", "train_file_mulan.arff -T ", "test_file_mulan.arff -x ",
                      "train_file_mulan.xml -o out.csv -c J48 -a ECC -u -b 75 -C ", chains, sep = "")
  }

  
  print(mulanstr)
  system(mulanstr)
  pred <- as.matrix(read.csv("pred_out.csv", header = FALSE))
...
chainsECCMulan <- function (nds_train, lbl_names, m = 10, seed = 1){  
  
  nmeta <- length(rownames(nds_train$labels)) - length(lbl_names)
  set.seed(seed)
  chains <- ""
  for (i in 1:m){
    chain = c(sample(1: nmeta), sample((nmeta + 1):length(rownames(nds_train$labels))))
    
    # java mulan is o indexed arrays
    chain <- chain -1
    
    chains <- paste(chains, paste(chain, collapse = "-"),"#", sep="")
  }
...
train_file<-"/home/leonardo/Downloads/mlds/5-fold/yeast/yeast_train_2"
test_file <-"/home/leonardo/Downloads/mlds/5-fold/yeast/yeast_test_2"
...
mulan <- "/usr/lib/jvm/java-1.8.0-openjdk-amd64/bin/java  -Xmx32g -jar /home/leonardo/Downloads/DownOlds/mulan/mymulanexec.jar"
mulan <- "/usr/lib/jvm/java-1.8.0-openjdk-amd64/bin/java  -Xmx32g -jar /home/leonardo/Downloads/mymulanexecute.jar"
dir <- tempdir()
setwd(dir)
...
if (cl == "ECC"){
  ...
  pred_puro = ECCMulan(dstrain, dstest, mulan)
  ...
  dstraint <- mldr_from_dataframe(cbind(dstrainatt, dslab), labelIndices = (ncol(dstrainatt)+1):(ncol(dstrainatt) + ncol(dslab)), name = "train")
  
  dstestatt <- dstest$dataset[,dstest$attributesIndexes]
  dstestt <- mldr_from_dataframe(cbind(dstestatt, dslabte), labelIndices = (ncol(dstestatt)+1):(ncol(dstestatt) + ncol(dslab)), name = "test")

  if (cl == "MLTB"){  
    
    pred <- ECCMulan(dstraint, dstestt, mulan)
    
    result_1 <- multilabel_evaluate(dstestt, pred, labels = T)
    
    # substituido pelo mulan
    ...
    colnames(pred) <- rownames(dstestt$labels)
    ...
    pred <- ECCMulan(dstraint, dstestt, mulan, chains = chainsECCMulan(dstraint, lbl_names = rownames(dstrain$labels)))
